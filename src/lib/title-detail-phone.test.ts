...
describe("title detail phone containment", () => {
  ...
  it("phone-stacks FieldList and title-detail ledgers from one SoT", () => {
    ...
    expect(TITLE_DETAIL_LEDGER_COPY_CLASS).toContain(HOUSE_PHONE_STACK_CLASS);
    expect(TITLE_DETAIL_LEDGER_COPY_CLASS).toContain("md:w-auto");
    expect(TITLE_DETAIL_LEDGER_COPY_CLASS).toContain("md:shrink-0");
    expect(TITLE_DETAIL_LEDGER_META_CLASS).toContain(HOUSE_PHONE_WRAP_CLASS);
    expect(TITLE_DETAIL_LEDGER_META_CLASS).not.toContain("shrink-0");
