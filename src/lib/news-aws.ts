...
export const NEWS_INGEST_SCHEDULE = "rate(30 minutes)";
...
/** Lambda may use the ingest role and omit static NEWS_AWS_* keys. */
export function isNewsIngestConfigured(env: NewsEnv = process.env): boolean {
  return Boolean(env.NEWS_AWS_REGION?.trim() && env.NEWS_DDB_TABLE?.trim());
}
...
export function newsAwsStaticCredentials(
  env: NewsEnv = process.env,
): { accessKeyId: string; secretAccessKey: string } | undefined {
  const accessKeyId = env.NEWS_AWS_ACCESS_KEY_ID?.trim();
  const secretAccessKey = env.NEWS_AWS_SECRET_ACCESS_KEY?.trim();
