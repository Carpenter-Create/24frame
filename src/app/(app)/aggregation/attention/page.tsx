...
// Attention = findings/health overview (§19). Former Catalog Health.
// A client org stays org-scoped. GC staff with no client org see the same
// findings UI across every org — my_findings already returns those rows
// because member_can('view') routes staff through gc_can, and
// titles/findings RLS already allow the queue's cross-org reads. Do not
  ...
  return (
    <>
      ...
      {findings.length === 0 ? (
        ...
        <div className="flex flex-col gap-[var(--space-6)]" data-attention="" data-catalog-health="">
          {Object.entries(byTitle).map(([titleId, items]) => {
            ...
            return (
              <Card key={titleId} data-attention-group="" data-catalog-health-group="">
                <CardBody>
