...
import AttentionPage from "./page";
...
/**
 * Attention has two modes. A client org stays org-scoped. GC staff with no
 * client org stay on /attention and see the existing findings UI across
 * every org — not /queue, not /, and not the wizard.
 */
describe("AttentionPage modes", () => {
  beforeEach(() => vi.clearAllMocks());

  it("renders GC-wide findings at /attention for staff with no client org", async () => {
    const { rpc } = stubClient();
    vi.mocked(getOrgContext).mockResolvedValue(
      ctx({ isGcStaff: true, orgStatus: null }) as never,
    );

    const html = renderToStaticMarkup(await AttentionPage());
  ...
  it("does not send GC staff with no client org to /queue, /, or the wizard", async () => {
    ...
    await expect(AttentionPage()).resolves.toBeTruthy();
  });
  ...
  it("sends a non-GC user with no org to the Aggregation empty home", async () => {
    ...
    await expect(AttentionPage()).rejects.toThrow("REDIRECT:/");
  });

  it("sends an unauthenticated visitor to login", async () => {
    stubClient();
    vi.mocked(getOrgContext).mockResolvedValue(null as never);
    await expect(AttentionPage()).rejects.toThrow("REDIRECT:/login");
  });
});
