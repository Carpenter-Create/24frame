...
export default async function EditVendorPage({
  ...
  const { data: vn } = await supabase
    .from("vendors")
    .select(
      "id, name, delivery_mode, email_to, email_cc, email_template, company_info, export_format_spec, active",
    )
    .eq("id", id)
    .maybeSingle();
  if (!vn) notFound();
    ...
    <>
      ...
      <VendorForm
        initial={{
          ...
          exportSpecJson: vn.export_format_spec
            ? JSON.stringify(vn.export_format_spec, null, 2)
            : "",
          active: vn.active,
        }}
      />
