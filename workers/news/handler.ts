...
// NEWS_AWS_* / NEWS_DDB_TABLE only. Never AWS_* / FINANCE_AWS_* /
// MEDIA_AWS_* / SES_AWS_* / EDUCATION_AWS_*. Founder-applied — see
// docs/infra/news-aws-setup.md. Do not create AWS from CI.

import { ingestNewsFeeds } from "../../src/lib/news-ingest";
import { createNewsStore } from "../../src/lib/news-store";
...
export async function handler(): Promise<{
  ...
  const summary = await ingestNewsFeeds({ store: createNewsStore() });
