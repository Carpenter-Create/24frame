...
Entry: `workers/news/handler.ts` calls `ingestNewsFeeds` in `src/lib/news-ingest.ts`. Fail-soft per source. Throws only when every live source failed so EventBridge can retry / DLQ.
